document.addEventListener('DOMContentLoaded', function() {
    fetch('http://localhost:8080/api/admin/roles')
        ...
        .then(roles => {
            const roleSelect = document.getElementById('roles_select');
            roles.forEach(role => {
    ...
    document.getElementById('newUser').addEventListener('submit', function(event) {
        ...
        fetch('http://localhost:8080/api/admin/users', {
            ...
            .then(() => {
                ...
                const allUsersPane = document.getElementById('users-table');
                const newUserTab = document.getElementById('nav-new-user-tab');
                const newUserPane = document.getElementById('new-user');
